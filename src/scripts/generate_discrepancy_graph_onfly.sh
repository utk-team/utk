...
discrepancy_pointset ()
{
	...
	if [ ! ${VERBOSE} = true ]
	then
		cmd="$DISCREPANCY -i pointsets_${SAMPLER_NAME}_n$1_${ID}.${FORMAT} -o ${TMPFILE} --silent"
	else
		cmd="$DISCREPANCY -i pointsets_${SAMPLER_NAME}_n$1_${ID}.${FORMAT} -o ${TMPFILE}"
		echo $cmd 
	fi
	eval $cmd
}
...
generate_plot ()
{
	...
	echo "plot \"${DISCREPANCY_GRAPH}\" u 1:5 w lp title \"$SAMPLER_NAME max discrepancy\" lw 2" >> gnuplot_${ID}.txt
# 	echo "set output \"${DISCREPANCY_PNG}\"" >> gnuplot_${ID}.txt
...
TMPFILE=`date "+%d-%m-%y"`"-generate_discrepancy_onfly-"${SAMPLER_NAME}"-"${DISCREPANCY_NAME}"-"${VALMIN}"-"${VALMAX}"-"${VALSTEP}"_${ID}.txt"
...
then
	...
	do
		...
		generate_pointset ${real_current}
		if [ ! $? = "0" ]
		then
			echo "Error while creating pointset ${real_current}"
			exit
		fi
