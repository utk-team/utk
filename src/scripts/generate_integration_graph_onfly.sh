...
integrate_pointset ()
{
	...
	if [ ! ${VERBOSE} = true ]
	then
		cmd="$INTEGRATION -i pointsets_${SAMPLER_NAME}_n$1_${ID}.${FORMAT} -o ${TMPFILE} --silent"
	else
		cmd="$INTEGRATION -i pointsets_${SAMPLER_NAME}_n$1_${ID}.${FORMAT} -o ${TMPFILE}"
		echo $cmd 
	fi
	eval $cmd
}
...
generate_plot ()
{
	...
	echo "plot \"${INTEGRATION_GRAPH}\" u 1:7 w lp title \"$SAMPLER_NAME MSE integration\" lw 2" >> gnuplot_${ID}.txt
	echo "set output \"${INTEGRATION_PNG}\"" >> gnuplot_${ID}.txt
