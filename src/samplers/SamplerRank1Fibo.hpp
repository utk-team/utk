#ifndef _UTK_SAMPLER_RANK1FIBO_
#define _UTK_SAMPLER_RANK1FIBO_
...
class SamplerRank1Fibo
{
	...
	public:
		...
		bool generateSamples(Pointset<D, T, P>& arg_pts, unsigned int arg_points)
		{
			if (!arg_pts.empty())
			{
				WARNING("SamplerRank1Fibo::generateSamples the pointset to fill is not empty, clearing it ...");
				arg_pts.clear();
			}
			
			if (D != 2)
			{
				WARNING("SamplerRank1Fibo::generateSamples is only a 2D sampler ...");
				return false;
			}

			if (!isFloatingType<T>())
			{
				ERROR("SamplerRank1Fibo::generateSamples generates samples in [0, 1]^d, can only run with points of type float or double");
				return false;
			}
			...
			std::vector<nsrank1fibo::Point> pts;
			int fib_lvl0 = 0;
			int fib_lvl1 = 1;
			int fib_lvl2 = 1;
			int i = 2;
            ...
            if(std::abs(fib_lvl2 - (int)arg_points) < std::abs(fib_lvl1 - (int)arg_points))
				pts = nsrank1fibo::fibonacci_rank1_lattices(i);
			else
				pts = nsrank1fibo::fibonacci_rank1_lattices(i-1);
