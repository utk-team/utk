...
/*const char source[] = BOOST_COMPUTE_STRINGIZE_SOURCE(__kernel void computeScoreMinDist(__global const unsigned int* permut_lsb,
                                                                              ...
                                                                              )
													{
														...
														for(uint i=1; i<tilesize-1; i++) 
														{ // start form the second i because xpermuts15bits[0].x = xpermuts15bits[0].y = 0;
														...
														for (uint ipts = 0; ipts < tilesize; ipts++) {
															for (uint ipts_to_compare = ipts+1; ipts_to_compare < tilesize; ipts_to_compare++) {
																...
																float dist_sq = 0;
																float dx = (float)permutted_pattern[2*ipts]/tilesize - (float)permutted_pattern[2*ipts_to_compare]/tilesize;
																dist_sq += dx*dx;
																float dy = (float)permutted_pattern[2*ipts+1]/tilesize - (float)permutted_pattern[2*ipts_to_compare+1]/tilesize;
																dist_sq += dy*dy;
																
																if (dist_sq < mindist) mindist = dist_sq;
															}
														}
														score[gid]= sqrt(mindist);
													}
                                                    );*/
...
/*std::vector<unsigned int> permutted_pattern_1;
std::vector<unsigned int> permutted_pattern_2;

void applyowen(std::vector<unsigned int> pattern, unsigned long long int permx, unsigned long long int permy)
{
	permutted_pattern_1.resize(128);
	...
	xpermuts[1] = permy;
 	...
 	for(uint ipt=0; ipt<tilesize; ipt++) {
 		...
 		for(uint ilevel=0; ilevel<NBITS; ilevel++) {
 			...
 			for(uint d=0; d<D; d++)
 			{
 				ind = (pattern[2*ipt+d] >> (NBITS-ilevel)) + (pow(2,ilevel)-1);
 				
				xresdigits[d].set(NBITS-ilevel-1, (xpermuts[d][PERMUTS_NBITS-ind-1] ^ thisSobolPt[d][NBITS-ilevel-1]) );
				//setIthDigit(xresdigits_int[d], NBITS-ilevel-1, (getIthDigit(xpermuts[d].to_ulong(),PERMUTS_NBITS-ind-1) ^ getIthDigit(thisSobolPt[d].to_ulong(),NBITS-ilevel-1)));
				
 			}
 		}
 		
 		for(uint d=0; d<D; d++)
			permutted_pattern_1[2*ipt+d] = (int)(xresdigits[d].to_ulong());
 	}
}


void applyMyOwen(std::vector<unsigned int> pattern, unsigned long long int permx, unsigned long long int permy)
{											
	unsigned int permut_x_lsb = (permx & 0xffffff);
	unsigned int permut_y_lsb = (permy & 0xffffff);
	unsigned int permut_x_msb = (permx >> 32);
	unsigned int permut_y_msb = (permy >> 32);
	
	permutted_pattern_2.resize(128);
	...
	for(uint ipt=0; ipt<tilesize; ipt++) {
		...
		for(uint d=0; d<2; d++)
			permutted_pattern_2[2*ipt+d] = xresdigits[d];
	}
}*/

/*bool permutFromNbitsToMbits_2(uint N, uint M, uint fixed_point[2], unsigned long long int permutN_x, unsigned long long int permutN_y, 
							unsigned long long int& permutM_x, unsigned long long int& permutM_y)
{
	unsigned int permutN_lsb[2];
	unsigned int permutN_msb[2];
	permutN_lsb[0] = (permutN_x & 0xffffff);
	permutN_lsb[1] = (permutN_y & 0xffffff);
	permutN_msb[0] = (permutN_x >> 32);
	permutN_msb[1] = (permutN_y >> 32);
	
	unsigned int permutM_lsb[2] = {0, 0};
	unsigned int permutM_msb[2] = {0, 0};
	...
	//permutM_x = permutM[0];
	//permutM_y = permutM[1];
	permutM_x = (unsigned long long int)((unsigned long long int)permutM_msb[0] << (unsigned long long int)(32)) + permutM_lsb[0];
	permutM_y = (unsigned long long int)((unsigned long long int)permutM_msb[1] << (unsigned long long int)(32)) + permutM_lsb[1];
	...
	else
	{
		...
		for(uint i=0; i<tilesize; i++)
		{
			for(uint j=(i+1); j<tilesize; j++)
			{
				...
				if(dx > 0 && dy >= 0) vector_angle = atan(dy/dx);
				else if(dx > 0 && dy < 0) vector_angle = atan(dy/dx) + 2*M_PI;
				else if(dx < 0) vector_angle = atan(dy/dx) + M_PI;
				else if(dx == 0 && dy > 0) vector_angle = M_PI/2.0;
				else vector_angle = 3*M_PI/2.0;
		...
		for(uint i=0; i<nbbins; i++)
		{
			...
			if(anisotropy_pattern[i] != 0 && fabs(1 - anisotropy_pattern[i]) > linf )
				ani_linf = fabs(1 - anisotropy_pattern[i]);
		}
...
int main(int argc, char** argv)
{
	...
	if(argc != 3)
	{
		std::cout << "Usage " << argv[0] << " <pattern_file> <pcf_file>" << std::endl;
		return 0;
	}
	...
	utk::Histogram1dd pcf_data;
	utk::HistogramReader<1, double, double> histo_reader;
	histo_reader.open(argv[2]);
	histo_reader.readHistogram(pcf_data);
	histo_reader.close();
	
	int nbbins = 20;
	assert(pcf_data.size() == nbbins);
	...
	final_perm_x = 0; final_perm_y = 0;
	permutFromNbitsToMbits(N, M, zero_pt, local_permutations_full[2*max_id], local_permutations_full[2*max_id+1], final_perm_x, final_perm_y);
	std::cout << "Done [" << time(NULL) - t << "s]" << std::endl;
	std::cout<< "[DEVICE] Best Permut " << final_perm_x << "," << final_perm_y << " (score " << max_score << ")" << "(dist " << max_dist << ")" << std::endl;

	
	std::string output_name(argv[1]);
	output_name+=".res";
	std::ofstream file(output_name);
	file << final_perm_x << " " << final_perm_y << std::endl;
	file.close();
