...
#include "SamplerVictorOwenPrime.hpp"
...
public:
	...
	bool generateSamples(Pointset<D, T, P>& arg_pts, unsigned long long int arg_points)
	{
		...
		for(uint current_dim=0; current_dim<2; current_dim+=2)
		{
		...
		for(uint current_dim=2; current_dim<D; current_dim+=2)
		{
			//uint level_max = ceil( log(arg_points)/log(tilesize) );
			//unsigned long long int nextpoweroftilesize = pow(tilesize, level_max);
			...
			indices[0] = sobol_ids[current_dim];
			indices[1] = sobol_ids[current_dim+1];
			owenprime.setIndices(indices, 2);

			if(!silent)
				std::cout << "Generating " << arg_points << " OwenPrime samples ..." << std::flush;
			if(!owenprime.generateSamples< 2, uint, Point<2, uint> >(input_pts, arg_points))
			{
				ERROR("SamplerVictor06022017::generateSamples Failed to generates sobol samples ...");
				return false;
			}
			...
			if(isIntegerType<T>())
			{
				...
				{
					arg_pts.domain.pMin.pos()[d] = 0;
					arg_pts.domain.pMax.pos()[d] = arg_points;
				}
				...
				for(unsigned long long int i=0; i<arg_points; i++)
				{
					for(int d=0; d<2; d++)
					{
						arg_pts[i].pos()[current_dim+d] = input_pts[i].pos()[d];
					}
				}
			}
			//Returns the pointset with float values in [0:1)
			else if(isFloatingType<T>())
			{
				...
				for(unsigned long long int i=0; i<arg_points; i++)
				{
					for(int d=0; d<2; d++)
					{
						arg_pts[i].pos()[current_dim+d] = input_pts[i].pos()[d]  / (double)input_pts.domain.pMax.pos()[d];
					}
				}
			}
			else
...
private:
	...
	bool recursivePermutations(uint levelmax, Pointset< 2, uint, Point<2, uint> >& input_pts, Pointset< 2, uint, Point<2, uint> >& output_pts)
	{
		...
		while(!stack.empty())
		{
			...
			if(!silent && (statsLUT[level][LUT_ACCESS]%100 == 0))
				std::cout << "Generating samples ... Level " << level << "/" << levelmax << "\t" <<  (double)statsLUT[level][LUT_ACCESS]/pow(tilesize, level) * 100.0 << "%...\r" << std::flush;
	...
	SamplerVictorOwenPrime owenprime;
