	...
	double				eps)			// error bound (ignored)
{
  s_PriSearchParams params;
	...
	params.ANNprPointMK = new ANNmin_k(k);		// create set for closest k points

										// distance to root box
	ANNdist box_dist = annBoxDistance(q,
				bnd_box_lo, bnd_box_hi, dim);

	params.ANNprBoxPQ = new ANNpr_queue(n_pts);// create priority queue for boxes
	...
	delete params.ANNprPointMK;				// deallocate closest point set
	delete params.ANNprBoxPQ;					// deallocate priority queue
}
