...
namespace utk
{
	...
	class SamplerRelaxedDartThrowing
	{
	protected:

	public:

		SamplerRelaxedDartThrowing() { setRandomSeedTime(); }
		...
		bool generateSamples(Pointset<D, T, P>& arg_pts, unsigned int arg_points)
		{
			if (!arg_pts.empty())
			{
				WARNING("SamplerRelaxedDartThrowing::generateSamples the pointset to fill is not empty, clearing it ...");
				arg_pts.clear();
			}
			
			if(!isFloatingType<T>())
			{
				ERROR("SamplerRelaxedDartThrowing::generateSamples generates samples in [0, 1]^d, can only run with points of type float or double");
				return false;
			}
			
			if(D < 2 || D > 8)
			{
				ERROR("SamplerRelaxedDartThrowing::generateSamples the maximum packing density has only been defined between 1 and 8 dimensions (http://mathworld.wolfram.com/HyperspherePacking.html)");
				return false;
			}
