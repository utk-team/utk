...
public:
	...
	bool generateSamples(Pointset<D, T, P>& arg_pts, unsigned int arg_points)
	{
		...
		double critfreq = 36.5;
		double peaksmooth = 2.0;
		double peakpower = 16.0;
		float smoothing = 8;
		Curve tmp(heck::nbins, 0, npts / 2);
		heck::FunctionPeak(critfreq, peakpower, tmp, pts.size());
		if (peaksmooth > 0.f)
				tmp = FilterGauss(heck::nbins, tmp, peaksmooth);
		Curve target = Power2RDF(npts, tmp, heck::nbins, 0, 0.5f, smoothing);
