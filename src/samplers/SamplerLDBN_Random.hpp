#ifndef _UTK_SAMPLER_LDBN_RANDOM_
#define _UTK_SAMPLER_LDBN_RANDOM_
...
public:
	...
	void setPermut()
	{
		...
		for (unsigned int y = 0; y < 128; y+=16)
		{
			...
			for(uint n=0; n<16; n++) std::swap(tab[rand()%16], tab[rand()%16]);
			...
			for(uint n=0; n<16; n++)
			{
				...
				O[i].pos()[0] = radicalInverse_VdC(y+off);
			}
		}
		...
		for (unsigned int x = 0; x < 128; x+=16)
		{
			...
			for(uint n=0; n<16; n++)
			{
				...
				O[i].pos()[1] = radicalInverse_VdC(x+off);
			}
		}
