#include "../../samplers/SamplerBNLDS_optimizer_GPU.hpp"
...
int main(int argc, char** argv)
{
	...
	if(param_K != 8)
	{
		std::cout << "[ERROR] OpenCL optimization only for pointset of size 64 (K=8)" << std::endl;
		exit(1);
	}
	...
	sampler.optiMinDist = true;
	sampler.fillEndTreeZero = true;
	
	sampler.optiPCF = false;
	sampler.fillEndTreeRandom = false;
	sampler.fillEndTreeRandomOpti = false;
