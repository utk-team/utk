...
int main(int argc, char** argv)
{
	...
	parser.addLongOption((char*)"shufflex", &param_shuffle[0], 1, assignDouble, displayDouble, (char*)"[uint]", (char*)"Shuffle X");
	parser.addLongOption((char*)"shuffley", &param_shuffle[1], 1, assignDouble, displayDouble, (char*)"[uint]", (char*)"Shuffle Y");
	parser.addLongOption((char*)"shufflez", &param_shuffle[2], 1, assignDouble, displayDouble, (char*)"[uint]", (char*)"Shuffle Z");
	...
	while(stream_in.readPointset(pts))
	{
	  for(int d=0; d<D; d++)
	    if(param_shuffle[d] == -1)
	      shuffle[d] = (int)( (double)getRandom01()*( (double)pts.domain.pMax.pos()[d]-(double)pts.domain.pMin.pos()[d]) );
		else
		  shuffle[d] = param_shuffle[d];
	    
	   pts.cranley_patterson(utk::Vector<D, unsigned int>(shuffle[0], shuffle[1]));
