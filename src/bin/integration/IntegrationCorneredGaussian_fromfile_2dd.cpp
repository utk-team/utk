...
int main(int argc, char** argv)
{
	...
	if(param_output_func)
	{
		...
		double ming = 10000;
		double maxg = 0;
		double shiftx=0.3417;
		double shifty=0.1618;
		...
		for(int j=0; j<512; j++)
		{
			...
			if(g > maxg) maxg = g;
			if(g < ming) ming = g;
			int x = i+shiftx; if (x > 512) x-=512;
			int y = j+shifty; if (y > 512) y-=512;
		    m_image_bw.atXY(x, y) = g;
		}
		for(int i=0; i<512; i++)
		for(int j=0; j<512; j++)
		{
		...
		m_image_bw.save("shifted_cornered_gauss.png");
		}
		{
		CImg<double> m_image_bw;
		m_image_bw = CImg<double>(512, 512, 1, 1);
		...
		m_image_bw.save("cornered_gauss.png");
		}*/
	}
