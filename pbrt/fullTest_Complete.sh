sampler_list="bnldseq16 bnldseq64"
pbrt_sampler_list="halton stratified sobol"
...
for s in ${pbrt_sampler_list}
do

for n in ${pts_list}
do

./testConfig.sh ${scene} ${s} ${n} ${imgsize} ${bounces}

file=${name}.png
filename=${name}_${s}_${n}spp_${bounces}bounces_Global_direct.png
mv ${file} ${filename}
...
mkdir renderings_${name}_full
mv *.png renderings_${name}_full
mv MSE.txt renderings_${name}_full