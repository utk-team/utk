sampler_list="bnldseq64 stratified halton sobol"
pts_list="4 16 64 256"
dimensions="image_dof_light"
...
file=${name}.png
filename=${name}_${s}_${n}spp_${tilesize}tilesize_${bounces}bounces_Global_6D.png
mv ${file} ${filename}
...
mkdir renderings_${name}_6D
mv *.png renderings_${name}_6D
mv MSE.txt renderings_${name}_6D